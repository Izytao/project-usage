...
// LimitFreqSingle
// 单机时间滑动窗口限流法: 限制某个操作在指定时间窗口内的最大访问次数，以防止过于频繁的请求。
func LimitFreqSingle(queueName string, count uint, timeWindow int64) bool {
	// 获取当前时间戳 currTime，用于记录当前请求的时间。
	...
	// 获取 queueName 对应的访问队列 q。
	q, _ := LimitQueue.readMap(queueName)
	//队列未满 ,队列 q 的长度小于允许的最大访问次数 count
	if uint(len(q)) < count {
		// 将当前时间戳 currTime 添加到队列末尾，并返回 true，表示请求通过。
